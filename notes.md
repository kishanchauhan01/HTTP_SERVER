...
### `NI_MAXHOST` is a constant used in C/C++ network programming to define the maximum buffer size required to store a fully qualified domain name (FQDN) or a numeric IP address

### NI_MAXSERV is a pre-defined constant in C networking libraries (found in headers like <netdb.h>) that specifies the maximum buffer size required to hold a service name string.
